package com.licker2689.lbp.functions;
...
public class LBPFunction {
    ...
    public static void getCoupon(CommandSender sender, String ssize, String target) {
        ...
        targetPlayer.getInventory().addItem(NBT.setInventoryTag(NBT.setIntTag(coupon, "lbp_size", size), Bukkit.createInventory(null, size * 9, "백팩 쿠폰"), "lbp_backpack"));
    ...
    public static void openBackPack(Player p, ItemStack item) {
        p.openInventory(NBT.getInventoryTag(item, "lbp_backpack"));
    }

    public static void saveBackPack(Player p, ItemStack item, Inventory inv) {
        p.getInventory().setItemInMainHand(NBT.setInventoryTag(item, inv, "lbp_backpack"));
    }
}
