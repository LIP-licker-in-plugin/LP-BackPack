package com.licker2689.lbp.commands;
...
public class LBPCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        if (args.length == 0) {
            sender.sendMessage(data.getPrefix() + "/lbp 쿠폰설정  - 백팩으로 사용할 아이템을 설정합니다.");
            sender.sendMessage(data.getPrefix() + "/lbp 쿠폰발급 <Size 1~6> (닉네임)");
            return false;
        }
        if (args[0].equalsIgnoreCase("쿠폰설정")) {
            ...
            if (args.length == 1) {
                LBPFunction.setCoupon((Player) sender);
                return false;
            }
        }
        if (args[0].equalsIgnoreCase("쿠폰발급")) {
            ...
            if (args.length == 2) {
                LBPFunction.getCoupon(sender, args[1], sender.getName());
                return false;
            }
            if (args.length == 3) {
                LBPFunction.getCoupon(sender, args[1], args[2]);
                return false;
            }
        }
        return false;
